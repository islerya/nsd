...
%
% Please read through the license and userguide before using. Also please cite the following 2 articles.
%
% Updated:
% Author: Yalcin Isler
% email: islerya@yahoo.com
% Release: 1.3
% Date: May 31, 2019
% Please cite: Gokcenur Gokce, Ilknur Veli, Yilmaz Kemal Yuce, Yalcin Isler (2020). Efficiency evaluation of rapid maxillary expansion treatment on nasal septal deviation using tortuosity ratio from cone-beam computer tomography images. Computer Methods and Programs in Biomedicine 188: 105260, https://doi.org/10.1016/j.cmpb.2019.105260.
%
% Original:
...
% Release: 1.2
% Date: July 31, 2010
% Please cite: Eric Johnston (2020). Interactive DICOM 3D Viewer. (https://www.mathworks.com/matlabcentral/fileexchange/28353-interactive-dicom-3d-viewer), MATLAB Central File Exchange. Retrieved April 20, 2020.
%
%
...
set(f,'Name','Islerya DICOM Viewer 1.0','Color',backgroundcolor);
...
%uicontrols
uicontrol('Style','Text','Position',[10 82 100 30],'String','Contrast:','FontSize',13,'BackgroundColor',backgroundcolor);
contrast = uicontrol('Style','Edit','Position',[120 88 40 24],'String',0.5,'FontSize',13,'Enable','off','BackgroundColor',backgroundcolor,'CallBack',@contrast_edit);
slide = uicontrol('Style','Slider','Position',[10 60 390 25],'Value',0.5,'Enable','off','CallBack',@slide_move);

lookfor  = uicontrol('Style','PushButton','Position',[10 10 100 50],'String','Find Directory','CallBack',@look_for_dicom);
...
rotate   = uicontrol('Style','PushButton','Position',[450 10 100 50],'String','Rotate','FontSize',13,'Enable','off','CallBack',@rotate_left);
export   = uicontrol('Style','PushButton','Position',[560 10 100 50],'String','Export','FontSize',13,'Enable','off','CallBack',@export_figure);
...
function look_for_dicom(src,eventdata) %#ok<INUSD> %Gather the dicom images into a coherent set
    try
        folder = uigetdir;
        w = cd; cd(folder);
    catch %#ok<CTCH>
    ...
    set(f, 'Pointer', 'arrow');
    set([zoomer measurer maximize contrast slide export],'Enable','on'); % rotate eklenmediði için Enable yapmadýk
    %Press Buttons
...
function export_figure(src,eventdata) %#ok<INUSD>
    if (gca==axial)
...
function measure_tool(src,eventdata) %#ok<INUSD> %Measures the distance between points
     ...
     measure_x1 = d(1,1); measure_y1 = d(1,2); %Get the anchor point
     if (gca == axial)
         if measure_x1>0 && measure_x1<length(axhorz) && measure_y1>0 && measure_y1<length(axvert)
            measure_lines = line([measure_x1 measure_x1], [measure_y1 measure_y1],'Color','Green');
            measure_direct = line([measure_x1 measure_x1], [measure_y1 measure_y1],'Color','Red');
            t = uicontrol('Style','text','String','','ForegroundColor','Green','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            td = uicontrol('Style','text','String','','ForegroundColor','Red','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            tn = uicontrol('Style','text','String','','ForegroundColor','Blue','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            set(f,'WindowButtonMotionFcn',@(h_obj,evt) measuredrag(t,td,tn,1));
            set(f,'WindowButtonUpFcn',@(h_obj,evt) measurestop(t,td,tn));
         end
     elseif (gca == sagittal)
         if measure_x1>0 && measure_x1<length(sagvert) && measure_y1>0 && measure_y1<length(saghorz)
            measure_lines = line([measure_x1 measure_x1], [measure_y1 measure_y1],'Color','Green');
            measure_direct = line([measure_x1 measure_x1], [measure_y1 measure_y1],'Color','Red');
            t = uicontrol('Style','text','String','','ForegroundColor','Green','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            td = uicontrol('Style','text','String','','ForegroundColor','Red','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            tn = uicontrol('Style','text','String','','ForegroundColor','Blue','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            set(f,'WindowButtonMotionFcn',@(h_obj,evt) measuredrag(t,td,tn,2));
            set(f,'WindowButtonUpFcn',@(h_obj,evt) measurestop(t,td,tn));
         end
     elseif (gca == coronal)
         if measure_x1>0 && measure_x1<length(corvert) && measure_y1>0 && measure_y1<length(corhorz)
            measure_lines = line([measure_x1 measure_x1], [measure_y1 measure_y1],'Color','Green');
            measure_direct = line([measure_x1 measure_x1], [measure_y1 measure_y1],'Color','Red');
            t = uicontrol('Style','text','String','','ForegroundColor','Green','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            td = uicontrol('Style','text','String','','ForegroundColor','Red','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            tn = uicontrol('Style','text','String','','ForegroundColor','Blue','BackgroundColor','Black','Position',[measure_x1 measure_y1 1 1],'Visible','off');
            set(f,'WindowButtonMotionFcn',@(h_obj,evt) measuredrag(t,td,tn,3));
            set(f,'WindowButtonUpFcn',@(h_obj,evt) measurestop(t,td,tn));
         end
     end
     set(f,'Pointer','arrow');
...
function measuredrag(t,td,tn,axestype) %Alter the measurement when the mouse drags
     set(f,'Pointer','crosshair');
     ...
     switch axestype
        case 1
            ...
            set(measure_lines(end),'XData',[measure_x1(end) x2],'YData',[measure_y1(end) y2]); %Update new line data points
            set(measure_direct,'XData',[measure_x1(1) x2],'YData',[measure_y1(1) y2]); %Update new line data points            
            distance = round(sqrt(xthickness*xthickness*(measure_x1(end)-x2)^2+ythickness*ythickness*(measure_y1(end)-y2)^2)*100)/100; %Calculate distance
            for k=2:length(measure_lines)
                distance = distance + round(sqrt(xthickness*xthickness*(measure_x1(k)-measure_x1(k-1))^2+ythickness*ythickness*(measure_y1(k)-measure_y1(k-1))^2)*100)/100;
            end
            distance2 = round(sqrt(xthickness*xthickness*(measure_x1(1)-x2)^2+ythickness*ythickness*(measure_y1(1)-y2)^2)*100)/100; %Calculate distance
            NSD = distance / distance2;
            set(t,'String',strcat(num2str(distance),'mm'),'Position',[dataf(1,1)+5 dataf(1,2)-20 60 15],'Visible','on'); 
            set(td,'String',strcat(num2str(distance2),'mm'),'Position',[dataf(1,1)+10 dataf(1,2)-35 60 15],'Visible','on'); 
            set(tn,'String',strcat('NSD=',num2str(NSD)),'Position',[dataf(1,1)+15 dataf(1,2)-50 60 15],'Visible','on'); 
        case 2
            if x2>length(sagvert), x2=length(sagvert); end, if y2>length(saghorz), y2=length(saghorz); end
            set(measure_lines(end),'XData',[measure_x1(end) x2],'YData',[measure_y1(end) y2]); %Update new line data points
            set(measure_direct,'XData',[measure_x1(1) x2],'YData',[measure_y1(1) y2]); %Update new line data points
            distance = round(sqrt(xthickness*xthickness*(measure_x1(end)-x2)^2+zthickness*zthickness*(measure_y1(end)-y2)^2)*100)/100;
            for k=2:length(measure_lines)
                distance = distance + round(sqrt(xthickness*xthickness*(measure_x1(k)-measure_x1(k-1))^2+ythickness*ythickness*(measure_y1(k)-measure_y1(k-1))^2)*100)/100;
            end
            distance2 = round(sqrt(xthickness*xthickness*(measure_x1(1)-x2)^2+ythickness*ythickness*(measure_y1(1)-y2)^2)*100)/100; %Calculate distance
        ...
        case 3
            ...
            set(measure_lines(end),'XData',[measure_x1(end) x2],'YData',[measure_y1(end) y2]); %Update new line data points
            set(measure_direct,'XData',[measure_x1(1) x2],'YData',[measure_y1(1) y2]); %Update new line data points
            distance = round(sqrt(ythickness*ythickness*(measure_x1(end)-x2)^2+zthickness*zthickness*(measure_y1(end)-y2)^2)*100)/100;
            for k=2:length(measure_lines)
                distance = distance + round(sqrt(xthickness*xthickness*(measure_x1(k)-measure_x1(k-1))^2+ythickness*ythickness*(measure_y1(k)-measure_y1(k-1))^2)*100)/100;
            end
            distance2 = round(sqrt(xthickness*xthickness*(measure_x1(1)-x2)^2+ythickness*ythickness*(measure_y1(1)-y2)^2)*100)/100; %Calculate distance
...
function measurestep()
     d = get(gca,'CurrentPoint');
     measure_x1(end+1) = d(1,1); measure_y1(end+1) = d(1,2); %Get the anchor point
     if (gca == axial)
         if measure_x1(end)>0 && measure_x1(end)<length(axhorz) && measure_y1(end)>0 && measure_y1(end)<length(axvert)
            measure_lines(end+1) = line([measure_x1(end) measure_x1(end)], [measure_y1(end) measure_y1(end)],'Color','Green');
         end
     elseif (gca == sagittal)
         if measure_x1(end)>0 && measure_x1(end)<length(sagvert) && measure_y1(end)>0 && measure_y1(end)<length(saghorz)
            measure_lines(end+1) = line([measure_x1(end) measure_x1(end)], [measure_y1(end) measure_y1(end)],'Color','Green');
         end
     elseif (gca == coronal)
         if measure_x1(end)>0 && measure_x1(end)<length(corvert) && measure_y1(end)>0 && measure_y1(end)<length(corhorz)
            measure_lines(end+1) = line([measure_x1(end) measure_x1(end)], [measure_y1(end) measure_y1(end)],'Color','Green');
         end
     end
end

function measurestop(t,td,tn)
    
     if strcmp( get(gcf,'Selectiontype') , 'normal') % add step measure when left-click
        measurestep();
        return
     end
     if strcmp( get(gcf,'Selectiontype') , 'alt') % stop measure when right-click
         set(f,'WindowButtonMotionFcn','','WindowButtonUpFcn','','Pointer','arrow');
         try 
         ...
         delete(measure_lines); delete(measure_direct); delete(t); delete(td); delete(tn); %Delete the line and text box showing distance
         measure_x1 = [];
         measure_y1 = [];
         measure_lines = [];
         measure_direct = [];
...
function rotate_left(src,eventdata) %#ok<INUSD> %Rotate left the images
     if (gca == axial)
         angle_axial = mod(angle_axial + 1, 4);
     elseif (gca == sagittal)
         angle_sagittal = mod(angle_sagittal + 1, 4);
     elseif (gca == coronal)
         angle_coronal = mod(angle_coronal + 1, 4);
     end
end
